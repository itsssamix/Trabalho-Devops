...
def create_order():
    ...
    inventory_response = requests.post(f"{INVENTORY_SERVICE_URL}/reserve", json=data)
    ...
    # Processar pagamento
    payment_response = requests.post(f"{PAYMENT_SERVICE_URL}/pay", json=data)
    if payment_response.status_code != 200:
        # Rollback
        requests.delete(f"{ORDER_SERVICE_URL}/orders/{order_response.json()['id']}")
        requests.post(f"{INVENTORY_SERVICE_URL}/release", json=data)
        return jsonify({"error": "Payment failed"}), 400
    
    return jsonify({"message": "Order created successfully", "order": order_response.json()}), 201
